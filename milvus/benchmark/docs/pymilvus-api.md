# PyMilvus API 与 DiskANN 参数指南

本文档介绍 PyMilvus 的基本 API 使用方法，以及 DiskANN 索引的参数配置与调优。

---

## 目录

1. [PyMilvus 基础 API](#1-pymilvus-基础-api)
2. [DiskANN 索引介绍](#2-diskann-索引介绍)
3. [DiskANN 参数详解](#3-diskann-参数详解)
4. [search_list 深度解析](#4-search_list-深度解析)
5. [参数调优指南](#5-参数调优指南)
6. [完整示例](#6-完整示例)

---

## 1. PyMilvus 基础 API

### 1.1 安装

```bash
pip install pymilvus
```

### 1.2 连接 Milvus

```python
from pymilvus import MilvusClient

# 方式1: 连接 Milvus Standalone/Cluster
client = MilvusClient(uri="http://localhost:19530")

# 方式2: 使用 Milvus Lite (本地嵌入式)
client = MilvusClient(uri="./milvus_lite.db")

# 获取服务器版本
print(client.get_server_version())
```

### 1.3 创建 Collection

```python
from pymilvus import DataType

# 创建 Schema
schema = client.create_schema(
    auto_id=False,           # 手动指定 ID
    enable_dynamic_field=False  # 禁用动态字段
)

# 添加字段
schema.add_field(
    field_name="id",
    datatype=DataType.INT64,
    is_primary=True          # 主键
)
schema.add_field(
    field_name="vector",
    datatype=DataType.FLOAT_VECTOR,
    dim=128                  # 向量维度
)
schema.add_field(
    field_name="text",
    datatype=DataType.VARCHAR,
    max_length=512           # 可选: 文本字段
)

# 创建索引参数
index_params = client.prepare_index_params()
index_params.add_index(
    field_name="vector",
    index_type="DISKANN",    # 索引类型
    metric_type="L2",        # 距离类型
    index_name="vector_index"
)

# 创建 Collection
client.create_collection(
    collection_name="my_collection",
    schema=schema,
    index_params=index_params
)
```

### 1.4 插入数据

```python
import numpy as np

# 准备数据
data = [
    {"id": i, "vector": np.random.random(128).tolist(), "text": f"doc_{i}"}
    for i in range(1000)
]

# 插入
client.insert(collection_name="my_collection", data=data)

# 刷新 (确保数据持久化)
client.flush(collection_name="my_collection")
```

### 1.5 加载与释放

```python
# 加载 Collection 到内存 (搜索前必须)
client.load_collection(collection_name="my_collection")

# 释放 Collection (释放内存)
client.release_collection(collection_name="my_collection")
```

### 1.6 向量搜索

```python
# 查询向量
query_vector = np.random.random(128).tolist()

# 搜索参数
search_params = {
    "metric_type": "L2",
    "params": {
        "search_list": 150  # DiskANN 特有参数
    }
}

# 执行搜索
results = client.search(
    collection_name="my_collection",
    data=[query_vector],     # 支持批量查询
    limit=10,                # Top-K
    search_params=search_params,
    output_fields=["text"]   # 返回的字段
)

# 解析结果
for hits in results:
    for hit in hits:
        print(f"ID: {hit['id']}, Distance: {hit['distance']}, Text: {hit['entity']['text']}")
```

### 1.7 删除与清理

```python
# 删除数据
client.delete(
    collection_name="my_collection",
    filter="id in [1, 2, 3]"  # 删除条件
)

# 删除 Collection
client.drop_collection(collection_name="my_collection")

# 关闭连接
client.close()
```

---

## 2. DiskANN 索引介绍

### 2.1 什么是 DiskANN

**DiskANN** (Disk-based Approximate Nearest Neighbor) 是微软研究院开发的一种**基于磁盘的向量索引算法**，专为大规模向量检索设计。

**核心特点**:
- 📀 **磁盘优化**: 将索引存储在 SSD 上，突破内存限制
- 🚀 **十亿级支持**: 可处理 1B+ 向量
- ⚡ **高性能**: 利用 NVMe SSD 实现低延迟搜索
- 🎯 **高召回率**: 通常 > 95%

### 2.2 DiskANN 工作原理

```
+=======================================================================================+
|                                  DiskANN 架构                                         |
+=======================================================================================+
|                                                                                       |
|   +------------------+        +------------------+        +------------------+        |
|   |     PQ 压缩      |        |    内存索引图     |        |     候选集合     |        |
|   |    (Product      | -----> |    (Vamana       | -----> |   (Candidate     |        |
|   |   Quantization)  |        |     Graph)       |        |      List)       |        |
|   +------------------+        +------------------+        +------------------+        |
|           |                          |                           |                   |
|           v                          v                           v                   |
|   +------------------+        +------------------+        +------------------+        |
|   |    SSD 存储      | <----> |    精确向量      | -----> |    最终结果      |        |
|   |  (Full Vectors)  |        |    (Rerank)      |        |    (Top-K)       |        |
|   +------------------+        +------------------+        +------------------+        |
|                                                                                       |
+=======================================================================================+
```

**搜索流程**:
1. **PQ 压缩距离计算**: 使用压缩向量快速筛选候选
2. **图遍历**: 在 Vamana 图上进行贪心搜索
3. **SSD 读取**: 从磁盘读取完整向量
4. **精排 (Rerank)**: 用原始向量计算精确距离
5. **返回 Top-K**: 输出最终结果

### 2.3 适用场景

| 场景 | 是否适合 | 说明 |
|------|---------|------|
| 向量数 > 100M | ✅ 非常适合 | DiskANN 核心优势 |
| 向量数 < 1M | ⚠️ 可选 | HNSW 可能更快 |
| 内存受限 | ✅ 非常适合 | 大部分数据在 SSD |
| 实时性要求极高 | ⚠️ 需权衡 | SSD I/O 有开销 |
| 高维向量 (>512) | ✅ 适合 | PQ 压缩效果好 |

---

## 3. DiskANN 参数详解

### 3.1 索引构建参数

在创建索引时可指定:

```python
index_params.add_index(
    field_name="vector",
    index_type="DISKANN",
    metric_type="L2",
    params={
        # 索引构建参数 (可选)
    }
)
```

| 参数 | 默认值 | 说明 |
|------|--------|------|
| `metric_type` | - | 距离类型: `L2`, `IP`, `COSINE` |

**距离类型说明**:

| 类型 | 全称 | 公式 | 适用场景 |
|------|------|------|----------|
| `L2` | 欧氏距离 | $\sqrt{\sum(a_i - b_i)^2}$ | 图像、SIFT 特征 |
| `IP` | 内积 | $\sum a_i \times b_i$ | 归一化向量、语义相似度 |
| `COSINE` | 余弦相似度 | $\frac{A \cdot B}{\|A\| \|B\|}$ | 文本嵌入、推荐系统 |

### 3.2 搜索参数

搜索时指定:

```python
search_params = {
    "metric_type": "L2",
    "params": {
        "search_list": 150  # 🔑 核心参数
    }
}
```

| 参数 | 类型 | 范围 | 默认值 | 说明 |
|------|------|------|--------|------|
| `search_list` | int | [topK, 65535] | 16 | 搜索时的候选列表大小 |

---

## 4. search_list 深度解析

### 4.1 什么是 search_list

`search_list` 是 DiskANN 搜索过程中**维护的候选向量队列大小**，它直接控制了搜索的**广度**和**精度**。

**类比理解**:
- 想象在迷宫中找最短路径
- `search_list` 就是你同时记住的"可能正确的岔路口"数量
- 记住的越多，找到最优路径的概率越高，但耗时也越长

### 4.2 search_list 工作机制

```
+=============================================================================+
|                         search_list 工作流程                                |
+=============================================================================+
|                                                                             |
|                            Query Vector                                     |
|                                 |                                           |
|                                 v                                           |
|   +---------------------------------------------------------------------+   |
|   |                      Vamana Graph 遍历                              |   |
|   |                                                                     |   |
|   |    [1] ---> [2] ---> [3] ---> [4] ---> [5] ---> ... ---> [N]       |   |
|   |     |       |        |        |        |                  |        |   |
|   |     +-------+--------+--------+--------+------------------+        |   |
|   |                                |                                   |   |
|   |                                v                                   |   |
|   |                  +---------------------------+                     |   |
|   |                  |      Candidate List       |                     |   |
|   |                  |  +---------------------+  |                     |   |
|   |                  |  | size = search_list  |  | <-- 核心参数        |   |
|   |                  |  | [v1, v2, ..., vN]   |  |                     |   |
|   |                  |  +---------------------+  |                     |   |
|   |                  +---------------------------+                     |   |
|   +---------------------------------------------------------------------+   |
|                                 |                                           |
|                                 v                                           |
|   +---------------------------------------------------------------------+   |
|   |               从 SSD 读取完整向量 (Rerank)                          |   |
|   |                    取前 TopK 个作为最终结果                          |   |
|   +---------------------------------------------------------------------+   |
|                                 |                                           |
|                                 v                                           |
|                          Final Results                                      |
|                                                                             |
+=============================================================================+
```

### 4.3 search_list 与性能的关系

**核心公式**:

$$
\text{Recall} \propto \log(\text{search\_list})
$$

$$
\text{Latency} \propto \text{search\_list}
$$

**实际影响**:

| search_list | 候选向量数 | 召回率 | QPS | 延迟 |
|-------------|-----------|--------|-----|------|
| 小 (50-100) | 少 | 较低 (~98%) | 高 | 低 |
| 中 (100-200) | 适中 | 高 (~99%) | 中 | 中 |
| 大 (200-500) | 多 | 很高 (~99.9%) | 低 | 高 |

### 4.4 search_list 对 GIST-960 的实测影响

来自我们的测试数据:

| search_list | QPS | Recall@100 | QPS 相对变化 |
|-------------|-----|------------|-------------|
| 100 | 9.66 | 99.92% | 基准 |
| 150 | 6.25 | 99.97% | -35% |
| 200 | 5.13 | 99.97% | -47% |
| 300 | 3.16 | 99.98% | -67% |

**可视化**:

```
QPS
 10 ┤ ●
  8 ┤   ╲
  6 ┤     ●
  5 ┤       ●
  4 ┤         ╲
  3 ┤           ●
  2 ┤
    └───────────────────
       100  150  200  300  → search_list
```

### 4.5 search_list 最佳实践

**选择原则**:

```python
# 1. 必须 >= TopK
search_list = max(topK, desired_list_size)

# 2. 推荐公式
search_list = topK * 1.5  # 平衡方案
search_list = topK * 2    # 高召回率方案
search_list = topK        # 高性能方案
```

**场景建议**:

| 场景 | 推荐 search_list | 说明 |
|------|-----------------|------|
| 实时推荐 | TopK ~ 1.2×TopK | 低延迟优先 |
| 语义搜索 | 1.5×TopK ~ 2×TopK | 平衡 |
| 高精度检索 | 2×TopK ~ 3×TopK | 召回率优先 |
| RAG 应用 | 1.5×TopK | 推荐 |

**代码示例**:

```python
def get_search_params(topK: int, priority: str = "balanced") -> dict:
    """根据优先级生成搜索参数"""
    multipliers = {
        "speed": 1.0,      # 速度优先
        "balanced": 1.5,   # 平衡
        "recall": 2.5      # 召回率优先
    }
    
    search_list = int(topK * multipliers.get(priority, 1.5))
    search_list = max(search_list, topK)  # 确保 >= topK
    
    return {
        "metric_type": "L2",
        "params": {"search_list": search_list}
    }

# 使用
params = get_search_params(topK=100, priority="balanced")  # search_list=150
```

---

## 5. 参数调优指南

### 5.1 性能调优流程

```
+=========================================================================+
|                          DiskANN 调优流程                               |
+=========================================================================+
|                                                                         |
|   Step 1: 确定目标                                                      |
|   +-------------------------------------------------------------------+ |
|   |  - 需要多高的召回率? (95%? 99%? 99.9%?)                           | |
|   |  - 可接受的延迟上限? (10ms? 50ms? 200ms?)                         | |
|   +-------------------------------------------------------------------+ |
|                                   |                                     |
|                                   v                                     |
|   Step 2: 基准测试                                                      |
|   +-------------------------------------------------------------------+ |
|   |  - 使用 search_list = TopK 测试基准性能                           | |
|   |  - 记录: QPS, Latency, Recall                                     | |
|   +-------------------------------------------------------------------+ |
|                                   |                                     |
|                                   v                                     |
|   Step 3: 二分调优                                                      |
|   +-------------------------------------------------------------------+ |
|   |  if Recall < 目标:                                                | |
|   |      search_list *= 1.5                                           | |
|   |  elif Latency > 目标:                                             | |
|   |      search_list *= 0.8                                           | |
|   |  重复直到满足条件                                                  | |
|   +-------------------------------------------------------------------+ |
|                                   |                                     |
|                                   v                                     |
|   Step 4: 验证与上线                                                    |
|   +-------------------------------------------------------------------+ |
|   |  - 长时间压测验证稳定性                                           | |
|   |  - 监控 P99 延迟                                                  | |
|   +-------------------------------------------------------------------+ |
|                                                                         |
+=========================================================================+
```

### 5.2 常见问题排查

| 问题 | 可能原因 | 解决方案 |
|------|---------|---------|
| 召回率低 | search_list 太小 | 增大 search_list |
| 延迟高 | search_list 太大 | 减小 search_list |
| QPS 低 | SSD 速度慢 | 使用 NVMe SSD |
| 索引构建慢 | 数据量大 | 增加内存，使用更快 SSD |
| 内存溢出 | 向量维度太高 | 考虑降维或使用更多内存 |

### 5.3 不同规模推荐配置

| 向量数 | 维度 | 内存建议 | search_list 建议 |
|--------|------|---------|-----------------|
| 1M | 128 | 8 GB | 100-200 |
| 1M | 960 | 16 GB | 100-150 |
| 10M | 128 | 16 GB | 150-250 |
| 100M | 128 | 32 GB | 150-300 |
| 1B | 128 | 64 GB | 200-400 |

---

## 6. 完整示例

### 6.1 完整的 DiskANN 使用示例

```python
"""
DiskANN 完整示例
"""

import numpy as np
from pymilvus import MilvusClient, DataType

# ========== 配置 ==========
MILVUS_URI = "http://localhost:19530"
COLLECTION_NAME = "diskann_demo"
DIMENSION = 128
NUM_VECTORS = 100000

# ========== 连接 ==========
client = MilvusClient(uri=MILVUS_URI)
print(f"Milvus 版本: {client.get_server_version()}")

# ========== 创建 Collection ==========
if client.has_collection(COLLECTION_NAME):
    client.drop_collection(COLLECTION_NAME)

schema = client.create_schema(auto_id=False, enable_dynamic_field=False)
schema.add_field("id", DataType.INT64, is_primary=True)
schema.add_field("vector", DataType.FLOAT_VECTOR, dim=DIMENSION)
schema.add_field("category", DataType.VARCHAR, max_length=64)

index_params = client.prepare_index_params()
index_params.add_index(
    field_name="vector",
    index_type="DISKANN",
    metric_type="L2"
)

client.create_collection(
    collection_name=COLLECTION_NAME,
    schema=schema,
    index_params=index_params
)
print(f"✅ 创建 Collection: {COLLECTION_NAME}")

# ========== 插入数据 ==========
BATCH_SIZE = 10000
categories = ["A", "B", "C", "D"]

for i in range(0, NUM_VECTORS, BATCH_SIZE):
    end = min(i + BATCH_SIZE, NUM_VECTORS)
    data = [
        {
            "id": j,
            "vector": np.random.random(DIMENSION).tolist(),
            "category": categories[j % len(categories)]
        }
        for j in range(i, end)
    ]
    client.insert(collection_name=COLLECTION_NAME, data=data)
    print(f"  插入: {end}/{NUM_VECTORS}")

client.flush(collection_name=COLLECTION_NAME)
print("✅ 数据插入完成")

# ========== 加载 ==========
client.load_collection(COLLECTION_NAME)
print("✅ Collection 已加载")

# ========== 搜索 ==========
query_vector = np.random.random(DIMENSION).tolist()
TOP_K = 10

# 不同 search_list 对比
for sl in [10, 50, 100, 200]:
    search_params = {
        "metric_type": "L2",
        "params": {"search_list": sl}
    }
    
    results = client.search(
        collection_name=COLLECTION_NAME,
        data=[query_vector],
        limit=TOP_K,
        search_params=search_params,
        output_fields=["category"]
    )
    
    print(f"\nsearch_list={sl}:")
    for hit in results[0][:3]:
        print(f"  ID: {hit['id']}, Distance: {hit['distance']:.4f}, Category: {hit['entity']['category']}")

# ========== 清理 ==========
client.release_collection(COLLECTION_NAME)
client.drop_collection(COLLECTION_NAME)
client.close()
print("\n✅ 清理完成")
```

### 6.2 性能测试模板

```python
"""
DiskANN 性能测试模板
"""

import time
import numpy as np
from pymilvus import MilvusClient

def benchmark_search(client, collection_name, queries, topK, search_list):
    """测试搜索性能"""
    search_params = {
        "metric_type": "L2",
        "params": {"search_list": search_list}
    }
    
    # 预热
    for _ in range(5):
        client.search(collection_name=collection_name, data=[queries[0]], 
                     limit=topK, search_params=search_params)
    
    # 正式测试
    start = time.time()
    for q in queries:
        client.search(collection_name=collection_name, data=[q], 
                     limit=topK, search_params=search_params)
    elapsed = time.time() - start
    
    qps = len(queries) / elapsed
    latency = (elapsed / len(queries)) * 1000
    
    return {"qps": qps, "latency_ms": latency}

# 使用
# results = benchmark_search(client, "my_collection", query_list, topK=100, search_list=150)
# print(f"QPS: {results['qps']:.2f}, Latency: {results['latency_ms']:.2f}ms")
```

---

## 参考资料

- [Milvus 官方文档](https://milvus.io/docs)
- [PyMilvus API 文档](https://milvus.io/api-reference/pymilvus/v2.4.x/About.md)
- [DiskANN 论文](https://proceedings.neurips.cc/paper/2019/file/09853c7fb1d3f8ee67a61b6bf4a7f8e6-Paper.pdf)
- [ann-benchmarks](https://ann-benchmarks.com/)
